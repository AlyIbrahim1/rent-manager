import type { CSSProperties, FormEvent, ReactNode } from "react";
import { AlertTriangle, ArrowRight, Eye, EyeOff, Loader2, Lock, Mail } from "lucide-react";

import {
  actionsWrapStyle,
  confirmPasswordFieldGroupStyle,
  devBannerStyle,
  dividerLineStyle,
  dividerTextStyle,
  dividerWrapStyle,
  errorBannerStyle,
  fieldGroupStyle,
  fieldLabelRowStyle,
  fieldLabelStyle,
  fieldWrapStyle,
  formStyle,
  googleButtonStyle,
  helperErrorTextStyle,
  inlineActionButtonStyle,
  inputStyle,
  leadingIconStyle,
  passwordFieldGroupStyle,
  passwordInputStyle,
  primaryButtonStyle,
  successBannerStyle,
  trailingIconButtonStyle,
} from "./authStyles";

export function AuthForm({
  children,
  onSubmit,
}: {
  children: ReactNode;
  onSubmit: (event: FormEvent<HTMLFormElement>) => void;
}) {
  return (
    <form style={formStyle} onSubmit={onSubmit}>
      {children}
    </form>
  );
}

export function EmailField({
  value,
  onChange,
  type = "email",
}: {
  value: string;
  onChange: (value: string) => void;
  type?: "email" | "text";
}) {
  return (
    <div style={fieldGroupStyle}>
      <label style={fieldLabelStyle} htmlFor="email">
        Email Address
      </label>
      <div style={fieldWrapStyle}>
        <span style={leadingIconStyle}>
          <Mail size={18} color="#999" />
        </span>
        <input
          id="email"
          name="email"
          type={type}
          autoComplete="email"
          value={value}
          onChange={(event) => onChange(event.target.value)}
          required
          placeholder="manager@domain.com"
          style={inputStyle}
          aria-label="Email address"
        />
      </div>
    </div>
  );
}

export function PasswordField({
  id = "password",
  label = "Password",
  value,
  onChange,
  showPassword,
  onToggleVisibility,
  autoComplete,
  minLength,
  headerAction,
  style,
}: {
  id?: string;
  label?: string;
  value: string;
  onChange: (value: string) => void;
  showPassword: boolean;
  onToggleVisibility: () => void;
  autoComplete: string;
  minLength?: number;
  headerAction?: ReactNode;
  style?: CSSProperties;
}) {
  return (
    <div style={id === "confirm-password" ? confirmPasswordFieldGroupStyle : passwordFieldGroupStyle}>
      {headerAction ? (
        <div style={fieldLabelRowStyle}>
          <label style={{ ...fieldLabelStyle, marginBottom: 0 }} htmlFor={id}>
            {label}
          </label>
          {headerAction}
        </div>
      ) : (
        <label style={fieldLabelStyle} htmlFor={id}>
          {label}
        </label>
      )}

      <div style={fieldWrapStyle}>
        <span style={leadingIconStyle}>
          <Lock size={18} color="#999" />
        </span>
        <input
          id={id}
          name={id}
          type={showPassword ? "text" : "password"}
          autoComplete={autoComplete}
          value={value}
          onChange={(event) => onChange(event.target.value)}
          required
          minLength={minLength}
          placeholder="••••••••"
          style={{ ...passwordInputStyle, ...style }}
          aria-label={label}
        />
        <button
          type="button"
          onClick={onToggleVisibility}
          style={trailingIconButtonStyle}
          aria-label={showPassword ? "Hide password" : "Show password"}
        >
          {showPassword ? <Eye size={18} color="#999" /> : <EyeOff size={18} color="#999" />}
        </button>
      </div>
    </div>
  );
}

export function InlineActionButton({
  children,
  onClick,
}: {
  children: ReactNode;
  onClick: () => void;
}) {
  return (
    <button type="button" onClick={onClick} style={inlineActionButtonStyle}>
      {children}
    </button>
  );
}

export function DevModeBanner() {
  return <div style={devBannerStyle}>Dev mode active</div>;
}

export function ErrorMessage({ message }: { message: string }) {
  return (
    <div role="alert" style={errorBannerStyle}>
      <AlertTriangle size={16} style={{ marginTop: 1, flexShrink: 0 }} />
      <p style={{ margin: 0, fontSize: 13, fontWeight: 500 }}>{message}</p>
    </div>
  );
}

export function InfoMessage({ message }: { message: string }) {
  return (
    <div role="status" style={successBannerStyle}>
      <p style={{ margin: 0, fontSize: 13, fontWeight: 500 }}>{message}</p>
    </div>
  );
}

export function HelperErrorText({ message }: { message: string }) {
  return <p style={helperErrorTextStyle}>{message}</p>;
}

export function SubmitButton({
  loading,
  label,
  loadingLabel = "Processing...",
}: {
  loading: boolean;
  label: string;
  loadingLabel?: string;
}) {
  return (
    <button
      type="submit"
      disabled={loading}
      style={{
        ...primaryButtonStyle,
        cursor: loading ? "not-allowed" : "pointer",
        opacity: loading ? 0.7 : 1,
      }}
    >
      {loading ? (
        <>
          <Loader2 size={18} className="animate-spin" color="#fff" />
          {loadingLabel}
        </>
      ) : (
        <>
          {label}
          <ArrowRight size={18} color="#fff" />
        </>
      )}
    </button>
  );
}

export function SocialDivider() {
  return (
    <div style={dividerWrapStyle}>
      <div style={dividerLineStyle} />
      <span style={dividerTextStyle}>OR</span>
    </div>
  );
}

export function GoogleButton({
  loading,
  label,
  onClick,
}: {
  loading: boolean;
  label: string;
  onClick: () => void;
}) {
  return (
    <button
      type="button"
      onClick={onClick}
      disabled={loading}
      aria-label={label}
      style={{
        ...googleButtonStyle,
        cursor: loading ? "not-allowed" : "pointer",
        opacity: loading ? 0.5 : 1,
      }}
    >
      <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true">
        <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
        <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
        <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
        <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
      </svg>
      {label}
    </button>
  );
}

export function AuthActions({ children }: { children: ReactNode }) {
  return <div style={actionsWrapStyle}>{children}</div>;
}
